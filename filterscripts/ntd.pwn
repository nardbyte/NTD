...
#define NDP_AUTO						(MAX_DIALOG_ITEMS)
...
#define IMPORTS_DIRECTORYPATH           "ntd/imports"           //Directory
...
#define IMPORTSLIST_FILEPATH            "ntd/imports.list" //File
...
enum E_TD
{
	...
	TD_BoxColorAlpha,
	TD_HighlightTimer,
	progressbar_direction:TD_BarDirectory
}
...
/*
			case DIALOG_OPEN:
			{
				if(response)
				{
					...
					if(listitem < Iter_Count(I_PROJECTS))
					{
					...
					{
						new importIndex = listitem - Iter_Count(I_PROJECTS);
						new impName[64];
						if(GetImportName(importIndex, impName))
						{
							if(LoadPwnImport(playerid, impName))
							{
								format(EditorLString, sizeof EditorLString, Language_Strings[str_infoprojectloaded]);
								strreplace(EditorLString, "#1", impName);
								ShowInfo(playerid, EditorLString);
								ShowWelcomeScreen(false);
								RelayerEditor();
								ShowEditorEx(playerid);
...
stock OpenTDDialog(playerid)
{
	...
	ShowPlayerDialogPages(playerid, "ManageTDs", DIALOG_STYLE_TABLIST_HEADERS,  EditorString, DLS[DL_TDLIST][d_s_button1], DLS[DL_TDLIST][d_s_button2], EditorMaxDialogItems);
	return 1;
}
...
stock OpenProjectDialog(playerid)
{
	...
	//printf("[NTD DEBUG] OpenProjectDialog called for player %d", playerid);
	//printf("[NTD DEBUG] I_PROJECTS count: %d", Iter_Count(I_PROJECTS));

	// First, check for imports and count them
	new File: impFile = fopen(IMPORTSLIST_FILEPATH, io_read);
	//printf("[NTD DEBUG] impFile handle: %d", _:impFile);

	if(impFile)
	{
		new line[64];
		while(fread(impFile, line))
		{
			for(new k=0; k<strlen(line); k++)
				if(line[k] == '\r' || line[k] == '\n') line[k] = 0;
			if(line[0])
			{
				import_count++;
				//printf("[NTD DEBUG] Found import: %s", line);
			}
		}
		fclose(impFile);
	}

	//printf("[NTD DEBUG] import_count: %d", import_count);

	new bool:hasContent = (Iter_Count(I_PROJECTS) > 0 || import_count > 0);
	//printf("[NTD DEBUG] hasContent: %d", _:hasContent);
	...
	if(hasContent)
	{
		...
		if(import_count > 0)
		{
			impFile = fopen(IMPORTSLIST_FILEPATH, io_read);
			if(impFile)
			{
				new line[64];
				while(fread(impFile, line))
		...
		//
		//printf("[NTD DEBUG] Total count before dialog: %d", count);
		CreateDialogCaptionOnLangData(DL_PROJECTSLIST);
		new tmp_str[2][24];
		format(tmp_str[0], 24, "%i", count);
		format(tmp_str[1], 24, "%i", MAX_NTD_PROJECTS);
		strreplace(EditorString, "#1", tmp_str[0]);
		strreplace(EditorString, "#2", tmp_str[1]);
		//printf("[NTD DEBUG] Showing dialog DIALOG_OPEN");
		ShowPlayerDialogPages(playerid, "OpenProject", DIALOG_STYLE_TABLIST_HEADERS, EditorString, DLS[DL_PROJECTSLIST][d_s_button1], DLS[DL_PROJECTSLIST][d_s_button2], EditorMaxDialogItems);
		PlayerSelectTD(playerid, false);
	}
	else
	{
		//printf("[NTD DEBUG] No content, showing info message");
...
stock LoadConfigurations()
{
	...
	if(dfile_FileExists(LANGUAGESLIST_FILEPATH))
	{
		...
		if(file)
		{
			...
			{
				freeSlot = Iter_Free(I_LANGUAGES);
				if(freeSlot != ITER_NONE)
				{
					if(sscanf(EditorString, "p<=>s[32]s[32]", Language[freeSlot][l_name], Language[freeSlot][l_file]) == 0)
					{
						Iter_Add(I_LANGUAGES, freeSlot);
						strdel(Language[freeSlot][l_file], strlen(Language[freeSlot][l_file]) - 2, strlen(Language[freeSlot][l_file]));
				...
				if(template_extract_mode)
				{
					...
					else
					{
						...
						if(mline[1] != -1)
						{
							...
							new templateSlot = Iter_Free(I_TEMPLATES);
							if(templateSlot != ITER_NONE)
							{
								Iter_Add(I_TEMPLATES, templateSlot);
								format(Template[templateSlot][Template_Name], 60, template_name);
								format(Template[templateSlot][Template_Data], 258, template_string);
							}
							else break;
							template_extract_mode = false;
							continue;
...
Float:TBarMaxPercentage = 100.0, progressbar_direction:TBarDirectory = BAR_DIRECTION_RIGHT)
{
...
stock GetImportName(index, name[], len=sizeof(name))
{
    new File: f = fopen(IMPORTSLIST_FILEPATH, io_read);
    if(!f) return 0;
    new line[64], count=0;
    while(fread(f, line)) {
        for(new k=0; k<strlen(line); k++)
...
stock LoadPwnImport(playerid, const name[])
{
    //printf("[NTD DEBUG] LoadPwnImport VERSION 5 LOADED");
    new path[128];
    format(path, sizeof path, "%s/%s.pwn", IMPORTS_DIRECTORYPATH, name);
    if(!dfile_FileExists(path)) return 0;

    new File:f = fopen(path, io_read);
    if(!f) return 0;

    foreach(new i : I_TEXTDRAWS) DestroyTD(i);
    NTD_User[User_ProjectOpened] = true;
    format(NTD_User[User_ProjectName], 128, "%s", name);

    new VarNames[MAX_NTD_TDS][32];
    new line[512], tmp[256];

    while(fread(f, line))
    {
        for(new k=0; k<strlen(line); k++) if(line[k] == '\r' || line[k] == '\n') line[k] = 0;
        if(!line[0]) continue;

        new type = 0;
        new idx = strfind(line, "TextDrawCreate");
        if(idx != -1) type = 1; else { idx = strfind(line, "CreatePlayerTextDraw"); if(idx != -1) type = 2; }
        ...
        if(type > 0)
        {
            ...
            new eq = strfind(line, "=");
            if(eq != -1 && eq < idx) {
                strmid(varname, line, 0, eq);
                for(new i=0; i<strlen(varname); i++) if(varname[i] == ' ' || varname[i] == '\t') strdel(varname, i, i+1), i--;
            }

            new op = strfind(line, "(", true, idx);
            new cp = strfind(line, ");", true, idx);
            if(cp == -1) cp = strfind(line, ")", true, idx);

            if(op != -1 && cp != -1)
            {
                strmid(tmp, line, op+1, cp);
                new Float:tx, Float:ty, ttext[300], ptrash[32];

                new success = 0;
                if(type == 1) { // Global: x, y, text
                     if(!sscanf(tmp, "p<,>ffs[300]", tx, ty, ttext)) success = 1;
                } else { // Player: pid, x, y, text
                     if(!sscanf(tmp, "p<,>s[32]ffs[300]", ptrash, tx, ty, ttext)) success = 1;
                }

                if(success)
                {
                    // Clean text quotes
                    new tlen = strlen(ttext);
                    if(tlen > 0 && ttext[0] == ' ') strdel(ttext, 0, 1), tlen--;
                    if(tlen > 1 && ttext[0] == '"') strdel(ttext, 0, 1), tlen--;
                    if(tlen > 0 && ttext[tlen-1] == '"') strdel(ttext, tlen-1, tlen);

                    new tdid = CreateNewTD(-1, "_", 0.0, 0.0, 0, (type == 1));
                    if(tdid != -1) {
                        NTD_TD[tdid][TD_PosX] = tx;
                        NTD_TD[tdid][TD_PosY] = ty;
                        format(NTD_TD[tdid][TD_Text], 300, "%s", ttext);
                        format(NTD_TD[tdid][TD_VarName], 35, "%s", varname); // Default varname
                        format(VarNames[tdid], 32, "%s", varname);
                        //printf("[NTD DEBUG] Created TD %d for var %s", tdid, varname);
                        DrawTD(tdid);
                    }
                }
            }
        ...
        else
        {
            ...
            {
                new funcname[64];
                strmid(funcname, line, 0, op);
                // Clean funcname
                for(new k=0; k<strlen(funcname); k++) if(funcname[k]==' '||funcname[k]=='\t') strdel(funcname, k, k+1), k--;

                new args[256];
                new cp = strfind(line, ");");
                if(cp == -1) cp = strfind(line, ")");
                if(cp != -1) strmid(args, line, op+1, cp);
                else continue;

                new var[32];
                new params_start_idx = 0;
                new is_player_td = (strfind(funcname, "PlayerTextDraw") != -1 && strfind(funcname, "Create") == -1);

                if(is_player_td)
                {
                    // PlayerTextDrawFunc(playerid, varname, params...);
                    new c1 = strfind(args, ",");
                    if(c1 != -1)
                    {
                        new c2 = strfind(args, ",", false, c1 + 1);
                        if(c2 != -1)
                        {
                            strmid(var, args, c1 + 1, c2);
                            params_start_idx = c2 + 1;
                        }
                    }
                }
                else
                {
                     // TextDrawFunc(varname, params...);
                     new c1 = strfind(args, ",");
                     if(c1 != -1)
                     {
                         strmid(var, args, 0, c1);
                         params_start_idx = c1 + 1;
                     }
                }
                ...
                {
                    // trim var
                    for(new k=0; k<strlen(var); k++) if(var[k]==' '||var[k]=='\t') strdel(var, k, k+1), k--;

                    // Find TDID
                    new tdid = -1;
                    foreach(new i : I_TEXTDRAWS) {
                         if(!strcmp(VarNames[i], var)) { tdid = i; break; }
                    }

                    //printf("[NTD DEBUG] Parsing Property: %s Args: %s Var: %s TDID: %d", funcname, args, var, tdid);

                    if(tdid != -1 && params_start_idx > 0)
                    {
                        // Parse remaining args
                        new params[200];
                        strmid(params, args, params_start_idx, strlen(args));
						// Params extracted

                        if(strfind(funcname, "TextSize") != -1) {
                            new Float:sx, Float:sy;
                            if(!sscanf(params, "p<,>ff", sx, sy)) {
                                NTD_TD[tdid][TD_BoxSizeX] = sx;
                                NTD_TD[tdid][TD_BoxSizeY] = sy;
                            }
                        }
                        if(strfind(funcname, "LetterSize") != -1) {
                             new Float:sx, Float:sy;
                             if(!sscanf(params, "p<,>ff", sx, sy)) {
                                 NTD_TD[tdid][TD_LetterSizeX] = sx;
                                 NTD_TD[tdid][TD_LetterSizeY] = sy;
                             }
                        }
                        if(strfind(funcname, "Alignment") != -1) {
                            new align = strval(params);
                            NTD_TD[tdid][TD_Alignment] = align;
                            //printf("[NTD DEBUG] Alignment set to %d", align);
                        }
                        if(strfind(funcname, "Color") != -1 && strfind(funcname, "Box") == -1 && strfind(funcname, "Back") == -1) {
                            // TextDrawColor
                             new col;
                             new hx = strfind(params, "0x");
                             if(hx != -1) sscanf(params[hx+2], "x", col);
                             else col = strval(params);

                             NTD_TD[tdid][TD_Color] = col;
                             NTD_TD[tdid][TD_ColorAlpha] = col & 0xFF;
                             //printf("[NTD DEBUG] Color set to %x", col);
                        }
                        if(strfind(funcname, "BoxColor") != -1) {
                             new col;
                             new hx = strfind(params, "0x");
                             if(hx != -1) sscanf(params[hx+2], "x", col);
                             else col = strval(params);

                             NTD_TD[tdid][TD_BoxColor] = col;
                             NTD_TD[tdid][TD_BoxColorAlpha] = col & 0xFF;
                             //printf("[NTD DEBUG] BoxColor set to %x", col);
                        }
                        if(strfind(funcname, "BackgroundColor") != -1) {
                             new col;
                             new hx = strfind(params, "0x");
                             if(hx != -1) sscanf(params[hx+2], "x", col);
                             else col = strval(params);

                             NTD_TD[tdid][TD_BGColor] = col;
                             NTD_TD[tdid][TD_BGColorAlpha] = col & 0xFF;
                             //printf("[NTD DEBUG] BGColor set to %x", col);
                        }
                        if(strfind(funcname, "Shadow") != -1) {
                             new size;
                             if(!sscanf(params, "i", size)) NTD_TD[tdid][TD_ShadowSize] = size;
                        }
                        if(strfind(funcname, "Outline") != -1) {
                             new size;
                             if(!sscanf(params, "i", size)) NTD_TD[tdid][TD_OutlineSize] = size;
                        }
                        if(strfind(funcname, "Font") != -1) {
                             new font;
                             if(!sscanf(params, "i", font)) NTD_TD[tdid][TD_Font] = font;
                        }
                        if(strfind(funcname, "SetProportional") != -1) {
                             new prop;
                             if(!sscanf(params, "i", prop)) NTD_TD[tdid][TD_Proportional] = (prop) ? true : false;
                        }
                        if(strfind(funcname, "SetSelectable") != -1) {
                             new sel;
                             if(!sscanf(params, "i", sel)) NTD_TD[tdid][TD_Selectable] = (sel) ? true : false;
                        }
                        if(strfind(funcname, "UseBox") != -1) {
                             new use;
                             if(!sscanf(params, "i", use)) NTD_TD[tdid][TD_UseBox] = (use) ? true : false;
                        }
                        // Preview Models? TextDrawSetPreviewModel
                        if(strfind(funcname, "SetPreviewModel") != -1) {
                            new model;
                            if(!sscanf(params, "i", model)) NTD_TD[tdid][TD_PrevModelID] = model;
                        }
...
DialogPages:OpenProject(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		if(listitem == INVALID_LISTITEM)
...
DialogPages:ManageTDs(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		if(listitem == INVALID_LISTITEM) return 0;
		if(listitem == 0) //Create New TD
		{
			CreateDialogOnLanguageData(playerid, DL_NEWTEXTDRAW);
			CreateDialogCaptionOnLangData(DL_NEWTEXTDRAW);
			ShowPlayerDialog(playerid, DIALOG_MANAGE3, DIALOG_STYLE_LIST, EditorString, EditorLString, DLS[DL_NEWTEXTDRAW][d_s_button1], DLS[DL_NEWTEXTDRAW][d_s_button2]);
